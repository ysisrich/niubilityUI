<template>
    <div class="nb-emotion-box" :style="[boxStyle,{width: `${width}px`,height: `${height}px` }]">
      <div class="nb-emotion-box-line" v-for="(line, i) in list" :key="i" >
        <span class="nb-emotion-item" v-for="(item, i) in line" :key="i" @click="clickHandler(item)" v-html="emotion(item)"></span>
      </div>
    </div>
</template>
...
export default {
  ...
  methods:{
    ...
    clickHandler(i){
        ...
        emotion_url = `<img src="https://res.wx.qq.com/mpres/htmledition/images/icon/emotion/${index}.gif" style="vertical-align: sub;" align="middle">`

        this.$emit('click', emotion,emotion_url)
    },
    // 显示表情包
    emotion(val){
        const name = val
